...
同步前 CI 会通过 HTTP/1.1 串行获取一次固定的 `bootable/recovery` revision，
并对瞬时 fetch 错误重试三次；随后全树同步使用 `--optimized-fetch`，不会再次
访问已验证的 recovery SHA。这样 GitLab 传输失败会在全量下载前显示为单项目错误。
